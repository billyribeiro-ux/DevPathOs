<script lang="ts">
  import { Folders, Plus, X, FloppyDisk, Trash, PencilSimple, Clock, CheckCircle, Pause, WarningCircle } from 'phosphor-svelte';
  ...
  const filtered = $derived(
    filterStatus === 'all' ? projectsList : projectsList.filter(p => p.status === filterStatus)
  );
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-muted-foreground">Track what you're building</p>
      </div>
    </div>
  ...
  {#if showForm}
    <div class="rounded-xl border border-border bg-card p-6 space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="font-semibold text-foreground">{editingProject ? 'Edit Project' : 'New Project'}</h3>
      ...
      <button onclick={saveProject} disabled={saving || !projectName} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
  ...
  <div class="flex gap-2 flex-wrap">
    {#each statuses as status}
      <button
        ...
        class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors {filterStatus === status ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}"
      >
  ...
  {#if filtered.length === 0}
    <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
      <Folders size={48} class="mx-auto mb-3 text-muted-foreground/50" />
      <p class="text-muted-foreground">{projectsList.length === 0 ? 'No projects yet. Start building!' : 'No projects match this filter.'}</p>
    </div>
  {:else}
      ...
      {#each filtered as project (project.id)}
        ...
        <div class="group rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/50 hover:shadow-sm">
          <div class="flex items-start justify-between mb-3">
            <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium {config.color}">
              <StatusIcon size={12} /> {config.label}
            </span>
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onclick={() => editProject(project)} class="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground"><PencilSimple size={14} /></button>
              <button onclick={() => deleteProject(project.id)} class="rounded p-1 text-muted-foreground hover:bg-red-500/10 hover:text-red-500"><Trash size={14} /></button>
            </div>
          </div>
