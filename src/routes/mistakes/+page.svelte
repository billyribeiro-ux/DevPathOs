      ...
      <div class="space-y-3">
        {#each filtered as mistake (mistake.id)}
          ...
          <div class="group rounded-xl border border-border bg-card p-4 transition-all hover:border-primary/30 hover:shadow-sm">
            ...
            <p class="text-sm text-foreground mb-1">{mistake.description}</p>
            <p class="text-xs text-muted-foreground mb-2">{new Date(mistake.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
