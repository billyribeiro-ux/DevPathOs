<script lang="ts">
  import { Bug, Plus, X, FloppyDisk, Trash, ChartBar } from 'phosphor-svelte';
  ...
  const filtered = $derived(
    filterCategory === 'all' ? mistakesList : mistakesList.filter(m => m.category === filterCategory)
  );

  function getCategoryConfig(cat: MistakeCategory) {
    return categories.find(c => c.value === cat) ?? categories[6];
  }

  const computedPatterns = $derived(
    (() => {
      const map = new Map<MistakeCategory, number>();
      for (const m of mistakesList) map.set(m.category, (map.get(m.category) ?? 0) + 1);
      return Array.from(map.entries())
        .map(([cat, count]) => ({ category: cat, count, config: getCategoryConfig(cat) }))
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold text-foreground">Mistake Tracker</h1>
        <p class="text-sm text-muted-foreground">Track error patterns and grow from them</p>
      </div>
    </div>
    <button onclick={() => { resetForm(); showForm = true; }} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
      <Plus size={16} /> Log Mistake
    </button>
  </div>

  <div class="flex gap-1 border-b border-border">
    <button onclick={() => { activeView = 'log'; }} class="border-b-2 px-4 py-2.5 text-sm font-medium transition-colors {activeView === 'log' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}">
      Mistake Log ({mistakesList.length})
    </button>
    <button onclick={() => { activeView = 'patterns'; }} class="flex items-center gap-1 border-b-2 px-4 py-2.5 text-sm font-medium transition-colors {activeView === 'patterns' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}">
      <ChartBar size={14} /> Patterns
    </button>
  </div>
  ...
  {#if showForm}
    <div class="rounded-xl border border-border bg-card p-6 space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="font-semibold text-foreground">Log a Mistake</h3>
        <button onclick={resetForm} class="text-muted-foreground hover:text-foreground"><X size={20} /></button>
      </div>
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          {#each categories as cat}
            <button
              onclick={() => { formCategory = cat.value; }}
              class="rounded-lg px-3 py-1.5 text-xs font-medium transition-colors {formCategory === cat.value ? cat.color + ' ring-2 ring-primary/30' : 'bg-muted text-muted-foreground'}"
            >
              {cat.label}
            </button>
          {/each}
      ...
      <button onclick={saveMistake} disabled={saving || !formDescription} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
  ...
  {#if activeView === 'log'}
    ...
    {#if filtered.length === 0}
      <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
        <Bug size={48} class="mx-auto mb-3 text-muted-foreground/50" />
        <p class="text-muted-foreground">No mistakes logged yet. That's either impressive or suspicious.</p>
      </div>
    {:else}
      <div class="space-y-3">
        {#each filtered as mistake (mistake.id)}
          {@const config = getCategoryConfig(mistake.category)}
          <div class="group rounded-xl border border-border bg-card p-4 transition-all hover:border-primary/30">
            <div class="flex items-start justify-between mb-2">
              <span class="rounded-full px-2 py-0.5 text-xs font-medium {config.color}">{config.label}</span>
              <button onclick={() => deleteMistake(mistake.id)} class="rounded p-1 text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-red-500/10 hover:text-red-500 transition-opacity"><Trash size={14} /></button>
            </div>
            <p class="text-sm text-foreground mb-2">{mistake.description}</p>
            {#if mistake.errorMessage}
              <pre class="rounded bg-muted/50 px-3 py-2 text-xs text-red-500 font-mono overflow-x-auto mb-2">{mistake.errorMessage}</pre>
            {/if}
            {#if mistake.resolution}
              <div class="rounded-lg bg-green-500/5 border border-green-500/20 px-3 py-2">
                <p class="text-xs font-medium text-green-500 mb-0.5">Resolution</p>
                <p class="text-sm text-foreground">{mistake.resolution}</p>
              </div>
            {/if}
          </div>
        {/each}
  ...
  {:else}
    {#if computedPatterns.length === 0}
      <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
        <p class="text-muted-foreground">Log some mistakes first to see patterns emerge.</p>
      </div>
    {:else}
      <div class="space-y-4">
        {#each computedPatterns as pattern}
          <div class="rounded-xl border border-border bg-card p-4">
            ...
            <div class="h-2 w-full rounded-full bg-muted overflow-hidden">
              <div class="h-full rounded-full bg-red-500/60" style="width: {Math.min(100, (pattern.count / mistakesList.length) * 100)}%"></div>
            </div>
            <p class="text-xs text-muted-foreground mt-2">
              {Math.round((pattern.count / mistakesList.length) * 100)}% of all mistakes
            </p>
          </div>
        {/each}
      </div>
