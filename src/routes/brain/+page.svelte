<script lang="ts">
  import { Brain, NotePencil, Lightning, Code, Plus, MagnifyingGlass, Trash, PencilSimple, FloppyDisk, X, Copy, Check, ArrowCounterClockwise } from 'phosphor-svelte';
...
<div class="space-y-6">
  ...
  <div class="flex items-center gap-3">
    <div class="relative flex-1">
      ...
      <input
        ...
        class="w-full rounded-lg border border-border bg-card py-2 pl-9 pr-3 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none"
      />
    </div>
    {#if activeTab === 'notes'}
      <button onclick={() => { resetNoteForm(); showNoteForm = true; }} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
        <Plus size={16} /> New Note
      </button>
    {:else if activeTab === 'flashcards'}
      <button onclick={() => { showCardForm = true; }} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
        <Plus size={16} /> New Card
      </button>
      {#if dueCards.length > 0}
        <button onclick={() => { reviewMode = true; currentCardIndex = 0; showAnswer = false; }} class="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">
          <ArrowCounterClockwise size={16} /> Review ({dueCards.length})
        </button>
      {/if}
    {:else}
      <button onclick={() => { showSnippetForm = true; }} class="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
  ...
  {#if activeTab === 'notes'}
    ...
    {#if filteredNotes.length === 0}
      <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
        <NotePencil size={48} class="mx-auto mb-3 text-muted-foreground/50" />
        <p class="text-muted-foreground">{search ? 'No notes match your search.' : 'No notes yet. Create your first note!'}</p>
      </div>
    {:else}
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {#each filteredNotes as note (note.id)}
          <div class="group rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/50 hover:shadow-sm">
            <div class="flex items-start justify-between mb-2">
              <h3 class="font-semibold text-foreground line-clamp-1">{note.title}</h3>
              <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onclick={() => editNote(note)} class="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground"><PencilSimple size={14} /></button>
                <button onclick={() => deleteNote(note.id)} class="rounded p-1 text-muted-foreground hover:bg-red-500/10 hover:text-red-500"><Trash size={14} /></button>
              </div>
            </div>
  ...
  {:else if activeTab === 'flashcards'}
    {#if reviewMode && dueCards.length > 0}
      ...
      <div class="mx-auto max-w-lg space-y-4">
        ...
        <div class="rounded-xl border border-border bg-card p-8 text-center min-h-[200px] flex flex-col items-center justify-center">
        ...
        {#if !showAnswer}
          <button onclick={() => { showAnswer = true; }} class="w-full rounded-lg bg-primary py-3 text-sm font-medium text-primary-foreground hover:bg-primary/90">
            Show Answer
          </button>
        {:else}
    ...
    {:else}
      ...
      <div class="grid grid-cols-3 gap-4">
        <div class="rounded-xl border border-border bg-card p-4 text-center">
          <p class="text-2xl font-bold text-blue-500">{newCards.length}</p>
          <p class="text-xs text-muted-foreground">New</p>
        </div>
        <div class="rounded-xl border border-border bg-card p-4 text-center">
          <p class="text-2xl font-bold text-amber-500">{dueCards.length}</p>
          <p class="text-xs text-muted-foreground">Due</p>
        </div>
        <div class="rounded-xl border border-border bg-card p-4 text-center">
          <p class="text-2xl font-bold text-green-500">{cardsList.length - dueCards.length - newCards.length}</p>
      ...
      {#if filteredCards.length === 0}
        <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
          <Lightning size={48} class="mx-auto mb-3 text-muted-foreground/50" />
          <p class="text-muted-foreground">{search ? 'No cards match your search.' : 'No flashcards yet. Create your first card!'}</p>
        </div>
      {:else}
        <div class="grid gap-3 md:grid-cols-2">
          {#each filteredCards as card (card.id)}
            <div class="group rounded-xl border border-border bg-card p-4 transition-all hover:border-primary/50">
              <div class="flex items-start justify-between mb-2">
                ...
                <button onclick={() => deleteCard(card.id)} class="rounded p-1 text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-red-500/10 hover:text-red-500 transition-opacity"><Trash size={14} /></button>
              </div>
  ...
  {:else}
    ...
    {#if filteredSnippets.length === 0}
      <div class="rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
        <Code size={48} class="mx-auto mb-3 text-muted-foreground/50" />
        <p class="text-muted-foreground">{search ? 'No snippets match your search.' : 'No snippets yet. Save your first code snippet!'}</p>
      </div>
    {:else}
      <div class="space-y-4">
        {#each filteredSnippets as snippet (snippet.id)}
          <div class="group rounded-xl border border-border bg-card overflow-hidden transition-all hover:border-primary/50">
            <div class="flex items-center justify-between border-b border-border px-4 py-2.5">
              ...
              <div class="flex gap-1">
                <button onclick={() => copyCode(snippet.code, snippet.id)} class="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                  {#if copiedId === snippet.id}<Check size={14} class="text-green-500" />{:else}<Copy size={14} />{/if}
                </button>
                <button onclick={() => deleteSnippet(snippet.id)} class="rounded p-1 text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-red-500/10 hover:text-red-500 transition-opacity"><Trash size={14} /></button>
              </div>
            </div>
